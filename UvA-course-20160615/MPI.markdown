...
# UvA course 2016-06-15  <br/> Extras - Wave simulation with MPI
...
* On the UI, import a new Ubuntu `appliance` from the AppMarket:
  * On the _appliances_ tab (under _<i class="fa fa-truck"></i> AppMarket_), import another copy of the **Ubuntu 14.04 Desktop CI** `appliance`. Make sure you select the proper datastore for your new OS image (**123:local_images_Cursus**). Give the `image` and `template` the name: **mpi_wave**
...
* Edit the `template` to make it a 2-core one:
  ...
  * Edit the _<i class="fa fa-globe"></i> Network_ tab:
    ...
    * add a second `nic` (hit **+ Add another interface** button); for this second `nic`, _NIC 1_, choose the _surfcourse-uva.int_ network (from the table on the right of the screen)
